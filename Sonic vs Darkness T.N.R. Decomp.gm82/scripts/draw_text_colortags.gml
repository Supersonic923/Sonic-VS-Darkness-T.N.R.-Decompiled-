/*
    ...
    CALL:
        draw_text_ext_colortags(x, y, str)

    INFO:
        Draws a text exactly like draw_text_ext, but with this
        script, you can use simple color tags to change the
        colors of certain words.

    EXAMPLE:

        draw_text_ext_colortags(x, y, "[c=255]Red text[/c], [c=$00FF00]green text[/c] and [c=c_blue]blue text[/c]!")
...
*/
{
    ...
    for (p = 1; p <= l; p += 1) {
        ...
        if (c == chr(10) || p == l || (c == '#' && string_char_at(str, p - 1) != '\')) {
            draw_text_ext_transformed(dx, dy, string_copy(str, dp, p - dp + 1),argument3,argument4,argument5,argument6,argument7);
            dp = p + 1;
            dx = xx;
            dy += h;
            ...
            if (p + string_length(st) <= l && string_copy(str, p, string_length(st)) == st) {
                ...
                draw_text_ext_transformed(dx, dy, ds,argument3,argument4,argument5,argument6,argument7);
                dx += string_width(ds);
                i = string_copy(str, p + stl, string_pos(et, string_delete(str, 1, p + stl)));
                p += string_length(i + et + st) - 1;
                if (i == '') i = pc;
                draw_set_color(execute_string('return ' + string(i)));
                dp = p + 1;
