 ...
 && lightdash=0 && homing=0 && nomove=false && nomovet=0 && stomplag=0 && auto_run=false
{
    ...
    instance_create(x,y,objStompWaveR)
    instance_create(x,y,objShadowWave)}
...
if ground=true && stomp=true && action=action_stomping
{
    ...
    instance_create(x,y+17,objSplashLandR)
    sound_play(sndStomp)}
    if slide=0 && angle=0
...
if action=action_stomping && animation!="bounce" && y_speed=0 && !instance_exists(objSplashLandR)
{
action=action_normal
stomp=false
}
...
instance_create(x,y,objStompTrailR)}}
else
