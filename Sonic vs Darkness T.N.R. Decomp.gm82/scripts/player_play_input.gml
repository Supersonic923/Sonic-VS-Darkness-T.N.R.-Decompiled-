frame_counter+=1
    ...
    */
    ...
    key_slide_released=false

    for (i=0i<keyr_a;i+=1)
    {
        if global.timeList[0,keyr_a] = frame_counter
        {
            key_action_direct=true
            key_action=true
        }
        else if key_action_direct=true
    ...
    //move lag
    if ground=true && (animation="idle" || animation="idle_wait") && action=action_normal && pauseplay=false
