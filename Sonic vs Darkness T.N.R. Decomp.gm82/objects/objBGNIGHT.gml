#define Create_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
//sound_stop(sndBGNight)
if !sound_isplaying(sndBGNight)
sound_loop(sndBGNight)
alarm[2]=random(30)+5
alarm[1]=5
alarm[0]=30
mastery=118
bossx=0
anspd=0
bgcolor=make_color_rgb(24,0,56)
//Sky 1
bg[0,0] = bg_night1s;//Index
bg[0,1] = 0;       //xmod
bg[0,2] = 246;     //width
bg[0,3] = 1;       //paralax value
bg[0,4] = 0+mastery;      //ymod
bg[0,5] = 0;        //xspeed

//Sky 2
bg[1,0] = sprL1BS;//Index
bg[1,1] = 0;       //xmod
bg[1,2] = 246;     //width
bg[1,3] = .995;       //paralax value
bg[1,4] = 113+mastery;      //ymod 8
bg[1,5] = 0;        //xspeed

//Carnival
bg[2,0] = bg_night5s;//Index
bg[2,1] = 0;       //xmod
bg[2,2] = 507;     //width
bg[2,3] = .92;       //paralax value
bg[2,4] = 276+mastery;      //ymod 13
bg[2,5] = 0;        //xspeed

//Castle
bg[3,0] = bg_night3s;//Index
bg[3,1] = 100;       //xmod
bg[3,2] = 260;     //width
bg[3,3] = .9;       //paralax value
bg[3,4] = 240+mastery;      //ymod 15
bg[3,5] = 0;        //xspeed

//Sky 3
bg[4,0] = bg_night4s;//Index
bg[4,1] = 0;       //xmod
bg[4,2] = 246;     //width
bg[4,3] = .88;       //paralax value
bg[4,4] = 291+mastery;      //ymod 13
bg[4,5] = 0;        //xspeed

//River 1
bg[5,0] = sprL1BW;//Index
bg[5,1] = 0;       //xmod
bg[5,2] = 256;     //width
bg[5,3] = .84;       //paralax value
bg[5,4] = 304+mastery;      //ymod
bg[5,5] = -.1;        //xspeed

//River 2
bg[6,0] = sprL1BW2;//Index
bg[6,1] = 0;       //xmod
bg[6,2] = 256;     //width
bg[6,3] = .82;       //paralax value
bg[6,4] = 312+mastery;      //ymod 8
bg[6,5] = -.15;        //xspeed

//River 3
bg[7,0] = sprL1BW3;//Index
bg[7,1] = 0;       //xmod
bg[7,2] = 256;     //width
bg[7,3] = .80;       //paralax value
bg[7,4] = 320+mastery;      //ymod 13
bg[7,5] = -.2;        //xspeed

//River 4
bg[8,0] = sprL1BW4;//Index
bg[8,1] = 0;       //xmod
bg[8,2] = 256;     //width
bg[8,3] = .78;       //paralax value
bg[8,4] = 328+mastery;      //ymod 15
bg[8,5] = -.3;        //xspeed

//Bush 1
bg[9,0] = bg_night1b;//Index
bg[9,1] = 0;       //xmod
bg[9,2] = 246;     //width
bg[9,3] = .75;       //paralax value
bg[9,4] = 341+mastery;      //ymod 16
bg[9,5] = 0        //xspeed

//Bush 2
bg[10,0] = bg_night2b;//Index
bg[10,1] = 0;       //xmod
bg[10,2] = 246;     //width
bg[10,3] = .7;       //paralax value
bg[10,4] = 347+mastery;      //ymod 16
bg[10,5] = 0        //xspeed

//Bush 3
bg[11,0] = bg_night3b;//Index
bg[11,1] = 0;       //xmod
bg[11,2] = 246;     //width
bg[11,3] = .65;       //paralax value
bg[11,4] = 359+mastery;      //ymod 16
bg[11,5] = 0        //xspeed

//Bush 4
bg[12,0] = bg_night4b;//Index
bg[12,1] = 0;       //xmod
bg[12,2] = 246;     //width
bg[12,3] = .6;       //paralax value
bg[12,4] = 375+mastery;      //ymod 16
bg[12,5] = 0        //xspeed

//Bush 5
bg[13,0] = bg_night5b;//Index
bg[13,1] = 0;       //xmod
bg[13,2] = 196;     //width
bg[13,3] = .5;       //paralax value
bg[13,4] = 399+mastery;      //ymod 16
bg[13,5] = 0        //xspeed

//Moon
bg[14,0] = bg_nightMoon;//Index
bg[14,1] = 325;       //xmod
bg[14,2] = 1000;     //width
bg[14,3] = .995;       //paralax value
bg[14,4] = 120+mastery;      //ymod 16
bg[14,5] = 0        //xspeed


background_hspeed[1]=-.2
background_hspeed[2]=-.3
background_hspeed[3]=-.1
#define Alarm_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=605
invert=0
arg0=Bottom Stage & Carnival
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
if bg[13,0] = bg_night5b{
bg[13,0] = bg_night5ba
bg[2,0] = bg_night5sa}
else{
bg[13,0] = bg_night5b
bg[2,0] = bg_night5s}

alarm[0]=30
#define Alarm_1
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=605
invert=0
arg0=Castle Light
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
if bg[3,0] = bg_night3s
bg[3,0] = bg_night3sa
else
bg[3,0] = bg_night3s

alarm[1]=5
#define Alarm_2
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=605
invert=0
arg0=Fireworks
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
if global.specialfx>2
repeat(choose(1,2,3,1))instance_create(view_xview+random(420),random(250)+150,objL1FW)
alarm[2]=random(30)+10
#define Draw_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=408
applies_to=self
invert=0
arg0=view_current=0 || view_current=7
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=422
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
draw_rectangle_color(view_xview[0],0,view_xview[0]+view_wview[0],room_height,bgcolor,bgcolor,bgcolor,bgcolor,false)
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
var ymod,x1,x2,bw,bx,by,i,h,xview;

anspd = (global.motion_time)*.008;

xview=view_xview[0]

ymod = view_yview[global.current_view] - (564-224)/(room_height-224)*view_yview[global.current_view];
x1 = view_xview[0];
x2 = view_xview[0]+640;

if view_current=0
for(h=0; h<15; h+=1){
    bg[h,1]+=bg[h,5];
    while(bg[h,1]<0)bg[h,1]+=bg[h,2];
    while(bg[h,1]>bg[h,2])bg[h,1]-=bg[h,2];
}

for(h=0; h<15; h+=1){
    bw = bg[h,2];
    bx = xview*bg[h,3]+bg[h,1];
    by = ymod+bg[h,4];

    i=x1-((x1 mod bw)-(bx mod bw))-bw*((x1 mod bw)<(bx mod bw));
    for(i=i; i<=x2; i+=bw){
    if h=14
    draw_set_blend_mode(bm_add)
    if (h>4 && h<9) || h=1
    draw_sprite(bg[h,0],anspd,floor(i),floor(by))
    else
        draw_background(bg[h,0],floor(i),floor(by));
        draw_set_blend_mode(bm_normal)
    }
}
