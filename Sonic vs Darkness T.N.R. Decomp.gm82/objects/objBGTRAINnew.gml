...
for(c=0;c<=h1*2;c+=h1){
for(i=0;i<=w1*2;i+=w1){
draw_background_ext(bg_train1,floor((view_xview+spd)*.8)+i,floor((view_yview+spd)*.5)+c,1,1,0,c_white,1)
draw_set_blend_mode_ext(bm_one,bm_src_alpha)
draw_background_ext(bg_train1,floor((view_xview+spd)*.8)+i,floor((view_yview+spd)*.5)+c,1,1,0,c_color,.6)}}
