...
*/
    ...
    // Rolling animation
    animation_rolling_start  = 31;
    animation_rolling_end    = 34;
    animation_rolling_loop   = 31;
    ...
    //Jump animation
    animation_jumping_start  = 31;
    animation_jumping_end    = 38;
    animation_jumping_loop   = 31;
    ...
    animation_stomp_land_start  = 270;
    animation_stomp_land_end    = 273;
    animation_stomp_land_loop   = 270;
    ...
    //Look Up
    animation_lookup_start = 151;
    animation_lookup_end   = 155;
    animation_lookup_loop  = 155;
    ...
    //Falling Frontwards
    animation_fallingfront_start = 256;
    animation_fallingfront_end   = 262;
    animation_fallingfront_loop  = 256;
    animation_fallingfront_speed = .25;
    animation_fallingfront_repeat = true;
    animation_fallingfront_linked_to = "(none)";

    //Falling Backwards
    animation_fallingback_start = 263;
    animation_fallingback_end   = 269;
    animation_fallingback_loop  = 263;
    ...
    //Freeze
    animation_freeze_start = 274;
    animation_freeze_end   = 279;
    animation_freeze_loop  = 274;
    animation_freeze_speed = .25;
    animation_freeze_repeat = true;
    animation_freeze_linked_to = "(none)";

    //Gresult
    animation_gresult_start = 246;
    animation_gresult_end   = 251;
    animation_gresult_loop  = 251;
    animation_gresult_speed = .2;
    animation_gresult_repeat = false;
    animation_gresult_linked_to = "(none)";

    //Bresult
    animation_bresult_start = 225;
    animation_bresult_end   = 244;
    animation_bresult_loop  = 225;
    animation_bresult_speed = .25;
    animation_bresult_repeat = false;
    animation_bresult_linked_to = "(none)";

    //Surf
    animation_surf_start = 252;
    animation_surf_end   = 253;
    animation_surf_loop  = 252;
    animation_surf_speed = .3;
    animation_surf_repeat = true;
    animation_surf_linked_to = "(none)";

    //Surf Jump
    animation_surfjump_start = 280;
    animation_surfjump_end   = 281;
    animation_surfjump_loop  = 280;
    animation_surfjump_speed = .3;
    animation_surfjump_repeat = true;
    animation_surfjump_linked_to = "(none)";

    //Surf Duck
    animation_surfduck_start = 282;
    animation_surfduck_end   = 283;
    animation_surfduck_loop  = 282;
    animation_surfduck_speed = .3;
    animation_surfduck_repeat = true;
    animation_surfduck_linked_to = "(none)";

    //Big Surf
    animation_big_surf_start = 254;
    animation_big_surf_end   = 255;
    animation_big_surf_loop  = 254;
    animation_big_surf_speed = .3;
    animation_big_surf_repeat = true;
    animation_big_surf_linked_to = "(none)";
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=605
invert=0
...
*/
    ...
    if transform=false{
    var camtrans;
    camtrans=instance_create(x,y,objSalomDie);
...
*/
 ...
 if objHandle!=noone
 {
        if action=action_stomping || animation="crouch_slide" || instance_exists(objGBoost) || action=action_soaring
        with objHandle
        instance_destroy()
 ...
 if objHandle!=noone
 {
        if action=action_stomping && (shield!=4 || !key_d_direct)
        with objHandle
        instance_destroy()
 }
        ...
        if (ground == false)
        {
            y   +=  y_speed;
                ...
                if (y_speed >= 0 && player_collision_bottom( x, y, 0, maskBig ) == true) && phase=false
                {
                    ...
                    y_speed =   0;
                    ground  =   true;
                    if surf=true && !instance_exists(objSurfSkid)
                     instance_create(x,y,objSurfSkid)
    ...
    // ========== PART 4 ==============================================================================================
        ...
        */
        ...
        if (ground == true && angle != 0 &&
            ...
            && player_collision_bottom( x, y, 0, maskBig ) == false
        {
            y_speed =   -sin(degtorad(angle))*x_speed;
            x_speed =   cos(degtorad(angle))*x_speed;
            ground  =   false;
...
// gameplay
        ...
        if (ground) && phase=false{
            ...
            if y_speed >= 0 && (player_collision_bottom(x,y,angle,maskBig) == false)// && edgefix=false
            {
                ground = false;
                player_set_angle(global.gravity_angle);
                //show_message("Somehow...")
            }
        }
        else if phase=false
        {
        ...
        if !instance_exists(objTarget) && goal=false && pauseplay=false && checknend=false
        {
            ...
            && path_index!=pathpush1)
            && (phase=false && visible=true)
            {
            action=action_normal
            animation="homeroll"
            instance_create(x-(1*animation_direction),y,objHomeAWave)
            ...
            y_speed=0
            airb=false
            sound_play(sndSidestep)}
        }
        if action=action_slash && animation_frame=150
        action=action_jumping
        ...
        if dist<250 && ((animation_direction = 1 && x<near_hom.x)||(animation_direction = -1 && x>near_hom.x))
        {
            ...
            && action!=action_hang && stomp=false && lightdash=0 && action!=action_rocket && action!=action_siderun && action!=action_pole && action!=action_twirl && action!=action_dive
            && (phase=false && visible=true) && path_index!=pathtrick1 && path_index!=pathtrick2 && surf=false
            {
              ...
              && !collision_line(x,y,near_hom.x,near_hom.y,objTargetBlockerV,true,true)
            {
                ...
                && action!=action_walljump  && action!=action_stomping && action!=action_hang && animation!="home" && action!=action_rocket && action!=action_dive
                && action!=action_siderun && action!=action_pole && action!=action_twirl && (phase=false && visible=true) && path_index!=pathtrick1 && path_index!=pathtrick2 && surf=false
                {
                if homing=0{
                sound_play(sndHomeAttack)
                instance_create(x-(1*animation_direction),y,objSalomWave)}
                homing=1;
                homehit=false
                }
            }
        ...
        if action!=action_hurt && goal=false && action!=action_dive && (phase=false && visible=true){
        ...
        if ground=false && (key_down_pressed || global.b_button=true) && action!=action_stomping && pauseplay=false && lightdash=0 && homing=0
        {
            ...
            instance_create(x,y,objStompWave)
            instance_create(x,y,objHomeWave2)
            x_speed=0
            y_speed=10
            stomp=true}
        ...
        if action=action_stomping && y_speed=0 && !instance_exists(objSplashLand)
        {
        action=action_normal
        stomp=false
        }
        ...
        instance_destroy()
        if action=action_stomping{
        if !instance_exists(objGBoost) && !instance_exists(objBoost)
        instance_create(x,y,objStompTrail)}
        ...
        {
            instance_create(x+15*animation_direction,y,objWJKick)
            action=action_normal
    ...
    sound_stop(sndWallSlide)
        ...
        {
            if !instance_exists(objBoost) && !instance_exists(objGBoost) && shield=0 && invincibility=0
            {
                instance_create(x,y,objJumpGlow)/*
                with instance_create(x,y,objJumpGlow){
                if objPlayer.x_speed > 10
                {
                image_xscale = objPlayer.x_speed/10
                image_yscale = objPlayer.x_speed/10
                }}*/
            }
            if (!key_action)
            {
        ...
        (action == action_hang && (hang=0 || hang=1 || hang=2 || hang=5) && canmove=true)
         || action == action_corkscrew_roll || action=action_rocket))
          ...
          && action!=action_elevator && action!=action_pipe && action!=action_siderun && action!=action_enter && action!=action_bigsurf)
          && checknend=false && pauseplay=false && goal=false && !place_meeting(x,y,objTestUnDuck) && !place_meeting(x,y,objRailDuck) && surfm=0 && bigone!=-1
        {
            ...
            if action=action_pole || action=action_surf
            y_speed=-5
            ...
            if animation != "walljump"{
            sound_play(sndJump);
            sound_play(choose(vcJump,vcJump2,snd,snd,snd))}
        ...
        if ( action == action_crouch_down )
        {
            ...
            // crouch down. while the character is quiet, on the ground and pressing down, activate the
            // crouch down token.
            if (ground == true && x_speed == 0 && key_down && action == action_normal) && pauseplay=false  && goal=false  && wheel=false
        ...
        if ( action == action_corkscrew || action == action_corkscrew_roll ) && cork="left/right"
        {
            ...
            {        
                ...
                angleInCorkscrew = degtorad((relativePosition/corkscrewObject.sprite_width)*360);
        ...
        else if ( abs(x_speed) >= 4 && ground == true && player_collision_main_object(x + x_speed, y, objSwirlPar) != noone )
        {
...
applies_to=self
*/
    if (instance_exists(objBoost) || instance_exists(objGBoost) || action=action_stomping || action=action_spindash || action=action_charge || abs(x_speed)>9 || abs(y_speed)>9 || homing=1)
    {
    if underwater=true
...
*/
        ...
        if (objHandle != noone)
        {
            ...
            else
            {
                ...
                || invincibility == 1 || invincibility == 3 || homing || shield==3) || (instance_exists(objBoost) || instance_exists(objGBoost))
                {
                    // Bounce
        ...
        if (objHandle != noone)
        {
            ...
            else
            {
                if (action == action_rolling || action == action_jumping || action == action_spindash || action == action_stomping || action=action_slash
                || (animation == "homeroll" || animation == "crouch_slide" || animation=="home")
                || invincibility == 1 || invincibility == 3 || homing=1 || shield==3) || (instance_exists(objBoost) || instance_exists(objGBoost))
                {
                    // Bounce
                    if (!ground && y_speed>0) && action!=action_stomping
                    {
        ...
        if (objHandle != noone)
        {
            ...
            else
            {
                if (action == action_rolling || action == action_jumping || action == action_spindash || action == action_stomping || action=action_slash
                || (animation == "homeroll" || animation == "crouch_slide" || animation=="home")
                || invincibility == 1 || invincibility == 3 || homing=1) || (instance_exists(objBoost) || instance_exists(objGBoost))
                {
                    // Bounce
        ...
        {
            var chose;
            chose=choose(0,0,0,1)
            if chose=0 || !instance_exists(objDarkChao){
            with(objHandle)
            {
                if motion=false{
                global.Score+=10;
...
if path_index=pathtrick1 || path_index=pathbutton1 || path_index=pathramp1 || path_index=pathpush1{
if path_position<.4
y_speed=-1
else
y_speed=1
...
//Bad Time
if key_action && (path_position<.45 || path_position>.6) && !instance_exists(objTrickSparkle){
...
if objController.blpha=1{
{
        ...
        if objController.randomkey1 = "A"{
            ...
            else if key_s_pressed || key_d_pressed || key_w_pressed && nchance=1{
...
applies_to=self
*/
if (((path_index=pathtrick1 || path_index=pathtrick2) && path_speed=10) || path_index=pathpush1 || lightdash=true || action=action_soaring || action=action_flightdash
...
&& !instance_exists(objGBoost) && !instance_exists(objBoost)
trail=true
else
trail=false
/*"/*'/**//* YYD ACTION
...
//---------------------------Gather Data----------------------------------------
    ...
    if global.HUB=false
    {
        ...
        else if instance_exists(objBoost){
        top_x_speed             =   15;
        max_x_speed             =   20;
        x_acceleration          =   0.1;
        }
...
if (player_collision_watr( x, y, angle, objWaterSolid )!=true) && phase=false
{
    ...
    floor(animation_frame)=25 || floor(animation_frame)=29 || floor(animation_frame)=159 || floor(animation_frame)=163 || floor(animation_frame)=169 ||
    floor(animation_frame)=40 || floor(animation_frame)=191 || floor(animation_frame)=193
    || floor(animation_frame)=209 || floor(animation_frame)=212)||
    floor(animation_frame)=175 ||
...
applies_to=self
*/
if surf=true && !instance_exists(objSurfSkid) && ground=true && slide=false && animation!="surfjump"
instance_create(x,y,objSurfSkid)
...
action=action_bigsurf
if x_speed<8 && action!=action_bigsurf
x_speed=8}
if bigone=-1 && action!=action_bigsurf
x_speed=10

if action=action_bigsurf
x_speed=0

if x_speed>12 && surf=true
x_speed=12

if action=action_surf && (ground=true||bigone=1) && pauseplay=false && bigone!=-1 && slide=false
{
if ((key_up_pressed && distance_to_object(objChaserCreator)>100) || bigone=1) && (surfp=0 || surfp=1) && surfm=0{
surfp=-1
surfm=10
y_speed=0
if bigone>0
...
if action=action_surf && (ground=true||bigone=2) && pauseplay=false && bigone!=-1 && slide=false
{
if ((key_down_pressed && distance_to_object(objChaserCreator)>100) || bigone=2) && (surfp=1 || surfp=2) && surfm=0{
surfp=-1
surfm=-12
y_speed=0
if bigone>0
...
y-=5
if surfm=0{
if y<=objSurfPlatM2N.y
surfp=1
if y<=objSurfPlatM3N.y
surfp=2
y+=2
ground=true
surfix=3}
...
if ground=false && surf=true && surfm=0 && surfix=0
animation="surfjump"
if ground=true && animation!="surfduck" && surf=true && y_speed=0
animation="surf"
...
*/
    ...
    if action=action_rolling && (keyboard_check_direct(vk_control)||keyboard_check_direct(ord('X')) || place_meeting(x,y,objTestUnDuck)) && abs(x_speed)>3
    ...
    if ( (action == action_normal || action=action_elevator) && ground == true)
    {
        ...
        else if abs(x_speed)>3 && (keyboard_check_direct(vk_control)||keyboard_check_direct(ord('X')) || place_meeting(x,y,objTestUnDuck)) && checknend=false && goal=false && pauseplay=false{
        ...
        else if (abs(x_speed)<=3 && abs(x_speed)>0.1) && (keyboard_check_direct(vk_control)||keyboard_check_direct(ord('X')) || place_meeting(x,y,objTestUnDuck))
        ...
        if ( action == action_enter && animation != "enter" && animation != "walking")
            animation = "enter";
        if ( action == action_surf && animation != "surf" && animation != "surfjump" && animation != "surfduck") && surf=true{
            animation = "surf";}
        if ( action == action_bigsurf && animation != "big_surf") && surf=true
            animation = "big_surf";
    }
if animation!="ground_roll" || animation_frame=205
groll=false

if animation_frame=273
animation="idle"
...
*/
        ...
        if homing=1
        instance_create(x,y,objHomeGlow)
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=605
invert=0
...
applies_to=self
invert=0
arg0=instance_exists(objBoost) || instance_exists(objGBoost)
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=532
...
with other{
if place_meeting(x,y,objRmp3)
global.buttonset=3
else if place_meeting(x,y,objRmp2)
global.buttonset=2
else
global.buttonset=1}

with other{
if place_meeting(x,y,objRmpM)
global.buttonlen=1
else
global.buttonlen=0}
...
*/
    ...
    if (invincibility == 2) && global.shield != 2
    {
            image_alpha=0.5
            if action=action_soaring || action=action_dive
            draw_sprite_ext(sprSalom, floor(animation_frame), floor(x), floor(y), image_xscale, image_yscale, image_angle, image_blend, hurt);
            else
            draw_sprite_ext(sprSalom, floor(animation_frame), floor(x), floor(y), animation_direction, 1, angle, image_blend, hurt);
    }
    else if (invincibility == 2) && global.shieldlayer=0 && global.shield == 2
    {
            image_alpha=0.5
            if action=action_soaring || action=action_dive
            draw_sprite_ext(sprSalom, floor(animation_frame), floor(x), floor(y), image_xscale, image_yscale, image_angle, image_blend, hurt);
    ...
    {
        if action=action_soaring || action=action_dive
        draw_sprite_ext(sprSalom, floor(animation_frame), floor(x), floor(y), image_xscale, image_yscale, image_angle, image_blend, 1);
        else
        draw_sprite_ext(sprSalom, floor(animation_frame), floor(x), floor(y), animation_direction, 1, angle, image_blend, 1);
        ...
        if (invincibility != 1 && global.shield == 1 && action != action_shield){
        draw_set_blend_mode(bm_add)
        draw_sprite_ext(sprShieldBetter, (global.motion_time div 50)*2+1, floor(x), floor(y),animation_direction,1,0,c_white,.6);
        draw_set_blend_mode(bm_normal)}

        if (invincibility != 1 && global.shield == 3 && action != action_shield){
        draw_set_blend_mode(bm_add)
        draw_sprite_ext(sprShieldLeaf, (global.motion_time div 50)*2+1, floor(x), floor(y),animation_direction,1,0,c_white,.6);
        draw_set_blend_mode(bm_normal)}
    ...
    {
        if room=rmSURF
        draw_sprite(sprMoveSign,1,view_xview+320,near.y)
        else
    ...
    {
        if room=rmSURF
        draw_sprite(sprMoveSign,2,view_xview+500,near3.y)
        else
    ...
    door=false
    if instance_exists(objTestZoneBooth)
    if distance_to_point(objTestZoneBooth.x,objTestZoneBooth.y)<50
    {
        draw_sprite(sprDoorArrow,global.motion_time div 120,view_xview+500,view_yview+50)
        door=true
    }
...
// TEST ZONE
else if room=rmHUBZone && distance_to_object(objTestZoneBooth)<10 && ground=true && abs(x_speed)<2
{
dontlook=true
if !instance_exists(objTestZoneC) && !instance_exists(objACConfirm){
instance_create(0,0,objTestZoneC)
x_speed=0}
}
...
applies_to=self
*/
if path_index!=pathtrick1&&path_index!=pathramp1&&path_index!=pathbutton1&&path_index!=pathpush1 && surf=false
{
if global.boost<80
if distance_to_object(objRingD) >= 48
if ground=true
if action=action_normal
if global.use14=true
if global.arcade=false
if checknend=false && action!=action_elevator && (global.HUB=false || room=rmHUB) && goal=false && pauseplay=false
if !instance_exists(objSalomTornado){
instance_create(x,y,objSalomTornado)
instance_create(x,y+16,objBoostDust)
...
applies_to=self
invert=0
arg0=checknend=false && action!=action_elevator && (global.HUB=false || room=rmHUB) && goal=false && pauseplay=false && bigwave=false && bigone!=-1
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
...
{
    if path_index!=pathtrick1&&path_index!=pathramp1&&path_index!=pathbutton1&&path_index!=pathpush1{
    if ground == true && pauseplay=false && (action!=action_pipe) && lightdash=false
    {
        if (animation_direction = 1 && key_right) && ((!key_down || animation="grindduck") && !key_up)
        {
            if green_boost = 1 && global.boost<92
            {
                if !instance_exists(objGBoost)
                instance_create(x,y,objGBoost)
                sound_play(choose(snd,vcBoost))
                if x_speed<20
                x_speed=20
            }
            else
    ...
    if ground=false && lightdash=false
    {
        if green_boost=0
        {
        if airb=true
        ...
        || action=action_spring_jump) && (phase=false && visible=true)
        {
            instance_create(x,y,objHomeWave)
            action=action_normal
            y_speed=0
            x_speed=7
        }
        ...
        else
        {
            if (animation_direction = 1 && key_right) && (!key_down && !key_up)
            && (action=action_jumping || action=action_normal || action=action_stomping || action=action_soaring || action=action_flightdash
            || action=action_spring_jump)  && global.boost<92 && (phase=false && visible=true)
            {
                if !instance_exists(objGBoost)
                instance_create(x,y,objGBoost)
                if y_speed<0
