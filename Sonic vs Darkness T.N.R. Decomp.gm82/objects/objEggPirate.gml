...
image_index=0
state=2;}
if state=2 && (image_index+image_speed)>=5{
dir *= -1;
state = 0;
x_speed =1.5*dir;
image_index=0}
...
image_speed=.2}
if state=1 && delay>0{
sprite_index=sprEggPirateR
image_speed=.2}
if state=1 && delay=0{
if sprite_index!=sprEggPirateS{
image_index=0
sound_play(sndEnemyMove)}
sprite_index=sprEggPirateS
image_speed=.3}
if state=2{
sprite_index=sprEggPirateT
image_speed=.2}
}
else
{
...
if state=1 && floor(image_index)=5 && delay=0
{
if attack=0
attack=instance_create(x+40*dir,y,objAttackMask)
}
if state=1 && floor(image_index)>5 && attack!=0 && delay=0
{
with attack
instance_destroy()
attack=0
...
// RETURN TO ROAMING
if image_index+image_speed>=9 && state=1{
x_speed =1.5*dir
state=0
image_index=0
dirz=0}
