#define Create_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
alph=1
back=false
alarm[0]=10
tran=false

image_speed=.3

get=false
rot=false

orbit_angle = 90;
orbit_radius = 40;
orbit_speed = -10;
XLen                    =   1;
YLen                    =   0;
#define Alarm_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
if distance_to_object(objPlayer)<600 && global.specialfx=3
instance_create(x+random(30)-15,y+random(30)-15,objEMS)
alarm[0]=10
#define Step_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
if back=false
alph-=.1
else
alph+=.1

if alph=.3
back=true
if alph=1
back=false
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
if instance_exists(objPlayer)
{
if rot=true{
//instance_create(x+YLen*4,y-XLen*4,objStarPFX);
orbit_angle += orbit_speed;
x = objPlayer.x + orbit_radius * cos(orbit_angle * pi / 180);
y = objPlayer.y - orbit_radius * sin(orbit_angle * pi / 180);
if tran=false
orbit_radius-=.25
else
orbit_radius-=tran
}
}
#define Collision_objPlayer
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=408
applies_to=self
invert=0
arg0=floor(orbit_radius)=0
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=422
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
/*if global.specialfx>2{
v=instance_create(x,y,objSpinGlowW)
v.image_blend=c_yellow}*/

//sound_play(sndEmeraldGet)
sound_play(sndSuperTransform)
global.emeralds[0]=1
global.emeralds[1]=2
global.emeralds[2]=3
global.emeralds[3]=4
global.emeralds[4]=5
global.emeralds[5]=6
global.emeralds[6]=7

with other
{
transform=true
global.trans_x = x;
global.trans_y = y;
global.player="Supersonic"
}

if global.specialfx>1{
instance_create(x,y,objSSTransform)
instance_create(x,y,objSSTransform2)}
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=202
applies_to=other
invert=0
arg0=objSupersonic
arg1=1
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=203
applies_to=self
invert=0
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=203
applies_to=objEmeraldSwirl
invert=0
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=424
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=421
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
rot=true
#define Draw_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
draw_set_blend_mode(bm_add)
if global.specialfx>1
draw_sprite_ext(sprEM3S,0,x,y,1,1,0,c_white,alph)
draw_set_blend_mode(bm_normal)
draw_sprite(sprite_index,image_index,x,y)

draw_set_blend_mode(bm_add)
if global.specialfx>1
draw_sprite_ext(sprEM3S,1,x,y,1,1,0,c_white,alph)
draw_set_blend_mode(bm_normal)
