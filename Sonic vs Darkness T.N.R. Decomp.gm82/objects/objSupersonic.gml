...
*/
    ...
    animation_walkingb_end    = 306;
    animation_walkingb_loop   = 305;
    ...
    animation_pushing_end    = 47;
    animation_pushing_loop   = 42;
    animation_pushing_speed  = 0.16;
    ...
    //Airfall animation
    animation_airfall_start  = 332;
    animation_airfall_end    = 334;
    animation_airfall_loop   = 334;
    ...
    animation_ramp_end    = 335;
    animation_ramp_loop   = 334;
    ...
    animation_waterfall3_start  = 291;
    animation_waterfall3_end    = 294;
    animation_waterfall3_loop   = 291;
...
{
    if global.Rings=0 && shield=0 && invincibility=0 && room!=rmTRAIN
    {
        instance_destroy()
    }
...
*/
         ...
         || ((shield == 3 || shield == 4) && action == action_shield) || action == action_corkscrew  || action == action_corkscrew_roll || slide=1) && checknend=false && pauseplay=false && goal=false && nomove=false && nomovet=0 && move_lag=0
        {
            ...
            {
                if animation="crouch_slide" || animation="skiddinga" && x_speed<0
                {
        ...
        {
            x_speed -=  sin(degtorad(angle)) * (slide_decc/1.5);
        }
        ...
        /*if (key_down || place_meeting(x,y,objRailDuck))&&animation!="corkscrew"{
        if animation!="grindduck"{
        animation="grindduck"
        rollcancel=true}}
        ...
        if (ground) && phase=false && fly=false
        {
            homehit=true
                motion_set(0,0);
                homing=0
        ...
        else if phase=false && animation!="climbrise" && animation!="climbrise2" && nofall=0 && fly=false
        {
            // Add gravity
...
*/
            ...
            && path_index!=pathbutton2) && (phase=false && visible=true) && nomove=false && nomovet=0 && auto_run=false
            {
                ...
                //else
                {
                    if !key_up && stomplag<3
                    {
                        ...
                        if !instance_exists(objSonicJump){
                        if global.specialfx>1
                            instance_create(x,y,objSonicJump)}
                            airhome=true
                    }
                    else if action=action_jumping && stomplag<3
                    {
                        action=action_normal
                        animation="home"
                        if global.specialfx>1
        ...
        if action!=action_hurt && goal=false && action!=action_dive && animation!="floating" && fly=false{
        ...
        if ground=false && (key_slide_pressed || global.b_button) && action!=action_stomping && (phase=false && visible=true) && pauseplay=false
    ...
    wjdelay=false}
        ...
        (action == action_hang && (hang=0 || hang=1 || hang=2 || hang=5) && canmove=true) || (action == action_pole_hang && !key_down)
         ...
         (animation="floating" && player_collision_top_obj(x,y-25,angle,maskMain,objFanAir)=false && player_collision_bottom_obj2(x,y+25,angle,maskMain,objFanAir)=true)))
          ...
          && action!=action_elevator && action!=action_pipe && action!=action_enter && animation!="ride2") && animation!="spinjump"
        ...
        if ( action == action_look_up )
        {
            ...
            if (key_up) && animation_reverse=true
                animation_reset=true
        } else {
        ...
        if ( action == action_crouch_down )
        {
            ...
            // while the character is crouching down, if the player doesn't pressed the key anymore,
            // deactivate
            if ( !key_down_direct && ground == true )
            {
                if animation_reverse=false{
            ...
            if (key_down) && animation_reverse=true
                animation_reset=true
            if ground=false
            action=action_normal
        } else {
...
*/
        ...
        if ( action == action_skid || action == action_skid2 )
        {
            ...
            && animation!="crouch_slide" && (key_left=false || key_right=false) && boss_boundary=false && nomovet=0 && auto_run=false
            {
...
*/
var dt;
dt= 12 //Adjustable delay between tricks
...
|| abs(x_speed)>6/*animation="sprinting"*/ || parkour=true) && !instance_exists(objBBoost) && !instance_exists(objBBoostTrail) && path_index!=pathramp1
trail=true
else
trail=false
/*"/*'/**//* YYD ACTION
...
flipdone2=true
    ...
    if action=action_rolling && (key_slide_pressed || global.b_button || player_collision_main_object(x, y, objTestUnDuck))
    ...
    if ( (action == action_normal || action=action_elevator) && ground == true) && animation!="crouch_slide" && animation!="crouch_slide2" && animation!="waterspout"
    {
        ...
        && checknend=false && goal=false && pauseplay=false && animation="crouch_slide" && boss_boundary=false && auto_run=false && !player_collision_watr(x,y,angle,objWaterSolid){
        if animation!="crouch_slide"{
        animation="crouch_slide"
        global.b_button=false
        if global.specialfx>1
        ...
        else if ( abs(x_speed) > 0 && abs(x_speed) <= 11 && animation != "walkingb" && animation!="spinboost" && animation != "crouch_slide2")
            animation = "walkingb";
...
*/
    ...
    */
    ...
    if ( animation != animation_previous ) || animation_reset=true
    {
        ...
        if (animation_set)
        player_sprite=sprSupersonic2
        else if action=action_swing
        player_sprite=sprSupersonicSwing
        else
        player_sprite=sprSupersonic
...
if hurt!=0
draw_sprite_general(sprSupersonic, floor(animation_frame)+floor(animation_frame2),0,0,sprite_width,sprite_height,floor(x) - lengthdir_x(32, 0)*animation_direction + lengthdir_y(37, 0)*animation_direction
...
*/
    ...
    if player_sprite=sprSupersonicSwing{
    if view_current=0
    other_frame+=.25
    animation_frame=other_frame}
        ...
        draw_set_blend_mode(bm_normal)}
        if !instance_exists(objBoostPar)
        draw_sprite_blend_ext(sprSSAura, (global.motion_time div 50)*1.5, floor(x), floor(y),1,1,0,c_white,.65,bm_add);
...
applies_to=self
invert=0
arg0=allowtransform=true && global.player=""
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=422
...
applies_to=self
invert=0
arg0=checknend=false && action!=action_elevator && (global.HUB=false || room=rmHUB) && goal=false && pauseplay=false && !player_collision_main_object(x, y, objTestUnDuck) && homing=0 && nomovet=0
*/
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
